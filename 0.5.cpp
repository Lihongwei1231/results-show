...
public:
    watchDerived():watch(){cout<<"watchDerived default constructor called"<<endl;}
	watchDerived(int h,int m,int s):watch(h,m,s){cout<<"watchDerived constructor called"<<endl;}
	void setzero();
...
{m_sec++;
 if(m_sec>=60)
 	{m_sec-=60;m_min++;if(m_min>=60)
 		{m_min-=60;m_hour=(m_hour+1)%12;}
	}
 return *this;
}
...
{
	out<<s.m_hour<<":"<<s.m_min<<":"<<s.m_sec<<endl;
